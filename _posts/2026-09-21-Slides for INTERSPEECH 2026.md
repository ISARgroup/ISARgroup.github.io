---
layout: post
title:  "Slides for INTERSPEECH 2026 Paper"
---

### Slides for INTERSPEECH 2026 Paper
### Label Correction Enhanced Dual-Stream Multiple Instance Learning for Weakly-Supervised Depression Detection in Speech

- Paper:
Sun Y, Zhou Y, Xu X, Qi J, Xu F, Ren Z, Schuller B. Label Correction Enhanced Dual-Stream Multiple Instance Learning for Weakly-Supervised Depression Detection in Speech[C]//Proc. INTERSPEECH, 2026. 

<br>

The slides can be downloaded via: 


Please contact: xinzhou.xu@njupt.edu.cn
